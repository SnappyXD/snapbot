...
try{
                zee.Facebook(`${q}`).then(async data => {
                    ...
                    txt += `*ï Title :* ${data.title}\n`
                    txt += `*ï Type :* ${data.medias[0].extension}\n`
                    txt += `*ï Quality :* ${data.medias[0].quality}\n`
                    txt += `*ï Size HD:* ${data.medias[1].formattedSize}\n`
					txt += `*ï Url :* ${data.url}`
                    let ppfb = await getBuffer(data.medias[1].url)
                    SnappyXD.sendMessage(from, ppfb, video, {mimetype:'video/mp4', quoted: mek, caption: txt})
             })} catch {
             	fakestatus('Fitur sedang error')
} 
   }          
             break
   case 'soundcloud':
				...
				zee.SoundCloud(`${q}`).then(async (data) => {
                    ...
                    txt += `*ï Title :* ${data.title}\n`
                    txt += `*ï Duration :* ${data.duration}\n`
					txt += `*ï Quality :* ${data.medias[1].quality}\n`
					txt += `*ï Ext :* ${data.medias[0].extension}\n`
                    txt += `*ï Size :* ${data.medias[0].formattedSize}\n`
                    txt += `*ï Url  :* ${data.url}\n\n`
	...
	case 'telesticker': case 'tstiker': {
			...
			SnappyXD.sendMessage(from, await getBuffer(telestc[i].url), sticker, {mimetype:'image/webp',quoted: mek})
			}
		}
		break
case 'tebakgambar':
...
fs.writeFileSync("./database/game/tebakgambar.json", JSON.stringify(tebakgambar))
console.log(jawaban)
SnappyXD.sendMessage(from, tebakya, image, {quoted: mek, caption: "\n\nTimeout : 120.00 seconds" })
await sleep(120000)
if (tebakgambar.hasOwnProperty(sender.split('@')[0])) {
SnappyXD.sendMessage(from, "Waktu permainan habis" + '\n\n' +"*Jawaban :*"  + '\n' + '_'+ jawaban +'_', text, {quoted: mek}) // ur cods
delete tebakgambar[sender.split('@')[0]]
fs.writeFileSync("./database/game/tebakgambar.json", JSON.stringify(tebakgambar))
                    }
                    })
...
fakestatus(lang.wait())
	emoji.get(`${qes}`).then(async emojii => {
					...
					console.log(teks)
					//SnappyXD.sendMessage(from, await getBuffer(teks), sticker, {mimetype:'image/webp',quoted: mek})
		  sendStickerFromUrl(from,`${teks}`)	
...
case 'ytmp3': {
			...
			await fakestatus(lang.wait())
                zee.Youtube(mulaikah).then(async (data) => {
                    ...
                    txt += `*ï Quality :* ${data.medias[7].quality}\n`
                    txt += `*ï Type :* ${data.medias[7].extension}\n`
                    txt += `*ï Size :* ${data.medias[7].formattedSize}\n`
                    txt += `*ï Url Source :* ${data.url}\n\n`
...
case 'ytmp4': {
			...
			zee.Youtube(mulaikah).then(async (data) => {
                    ...
                    txt += `*ï Quality :* ${data.medias[1].quality}\n`
                    txt += `*ï Type :* ${data.medias[1].extension}\n`
                    txt += `*ï Size :* ${data.medias[1].formattedSize}\n`
                    txt += `*ï Url Source :* ${data.url}\n\n`
                    txt += `_Mohon tunggu sebentar , uploading media..._`
                    sendFileFromUrl(from, data.thumbnail, txt, mek)
...
ï Title : ${aramas.videos[0].title}
ï ID : ${aramas.videos[0].videoId}
ï Upload : ${aramas.videos[0].ago}
ï Size : ${data.medias[7].formattedSize}
ï Views: ${aramas.videos[0].views} 
ï Duration : ${aramas.videos[0].timestamp}
ï Url : ${aramas.videos[0].url}`
